...
/*   Updated: 2025/05/05 19:07:46 by albermud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_start(t_mini *shell, t_token *token, t_token	*next)
{
    ...
    if (next && is_redirect_type(next->type))
    {
        if (!redir(shell, next))
        {
            shell->exit_code = 1;
            shell->exit = false;
            // When redirection fails, we need to ensure all pipe processes are cleaned up
            close(shell->pipin);  // Close any open pipe
            return ;
        }
        exec_start(shell, token, next->next);
        return ;
